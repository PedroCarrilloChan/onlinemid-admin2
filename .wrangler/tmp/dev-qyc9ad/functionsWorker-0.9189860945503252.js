...
// .wrangler/tmp/bundle-ijMbkY/checked-fetch.js
...
// .wrangler/tmp/pages-xEoJ0n/functionsWorker-0.9189860945503252.mjs
...
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    if (!email || !password) {
      return new Response(JSON.stringify({ error: "Correo y contrase\xF1a son requeridos" }), {
        status: 400,
...
var onRequestGet = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    console.error("Error en API p\xFAblica:", error);
    return new Response(JSON.stringify({ error: "Error interno del servidor" }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
...
var onRequestGet2 = /* @__PURE__ */ __name2(async ({ env }) => {
  try {
    ...
    const db = env.DB;
    const allUsersQuery = db.prepare("SELECT id, email, createdAt FROM Usuarios");
    const usersResult = await allUsersQuery.all();
...
var PasswordHash = class {
  static {
    __name(this, "PasswordHash");
  }
  static {
    __name2(this, "PasswordHash");
  }
  ...
  static async verifyPassword(password, hashedPassword) {
    try {
      const combined = new Uint8Array(
        atob(hashedPassword).split("").map((char2) => char2.charCodeAt(0))
      );
...
var CloudflareD1Storage = class {
  static {
    __name(this, "CloudflareD1Storage");
  }
  constructor(db) {
    this.db = db;
  }
  static {
    __name2(this, "CloudflareD1Storage");
  }
  // D1Database type
  async getUser(id) {
    try {
      const result = await this.db.prepare("SELECT id, username FROM users WHERE id = ?").bind(id).first();
  ...
  async getUserByUsername(username) {
    try {
      const result = await this.db.prepare("SELECT id, username FROM users WHERE username = ?").bind(username).first();
      return result ? result : void 0;
    } catch (error) {
  ...
  async getUserWithPassword(username) {
    try {
      const result = await this.db.prepare("SELECT id, username, password FROM users WHERE username = ?").bind(username).first();
      return result ? result : void 0;
    } catch (error) {
  ...
  async createUser(insertUser) {
    try {
      ...
      const result = await this.db.prepare("INSERT INTO users (id, username, password) VALUES (?, ?, ?)").bind(id, insertUser.username, passwordHash).run();
      if (!result.success) {
  ...
  async verifyPassword(username, password) {
    try {
      ...
      if (!user) return null;
      const isValid2 = await PasswordHash.verifyPassword(password, user.password);
      if (isValid2) {
...
var CloudflareKVStorage = class {
  static {
    __name(this, "CloudflareKVStorage");
  }
  constructor(usersKV, usernameIndexKV) {
    this.usersKV = usersKV;
    this.usernameIndexKV = usernameIndexKV;
  }
  static {
    __name2(this, "CloudflareKVStorage");
  }
  async getUser(id) {
    try {
  ...
  async getUserWithPassword(username) {
    try {
      ...
      return userData ? userData : void 0;
    } catch (error) {
...
var CloudflareMemStorage = class _CloudflareMemStorage {
  static {
    __name(this, "_CloudflareMemStorage");
  }
  static {
    __name2(this, "CloudflareMemStorage");
  }
  static users = /* @__PURE__ */ new Map();
  static usernameIndex = /* @__PURE__ */ new Map();
  // username -> id
  async getUser(id) {
    return _CloudflareMemStorage.users.get(id);
  }
  async getUserByUsername(username) {
    const userId = _CloudflareMemStorage.usernameIndex.get(username);
    if (!userId) return void 0;
    return _CloudflareMemStorage.users.get(userId);
  }
  async createUser(insertUser) {
    try {
      if (_CloudflareMemStorage.usernameIndex.has(insertUser.username)) {
        throw new Error("Username already exists");
      }
      const passwordHash = await PasswordHash.hashPassword(insertUser.password);
      const id = crypto.randomUUID();
      ...
      _CloudflareMemStorage.users.set(id, user);
      _CloudflareMemStorage.usernameIndex.set(user.username, id);
      return user;
    } catch (error) {
      if (error.message === "Username already exists") {
        throw error;
...
var errorResponse = /* @__PURE__ */ __name2((message, status = 500) => {
...
var routes2 = [
  // Get user by ID: GET /api/users/:id
  {
    method: "GET",
    path: "/api/users/:id",
    handler: /* @__PURE__ */ __name2(async (context, params) => {
...
async function onRequest(context) {
  ...
  try {
    const match2 = matchRoute(method, apiPath);
    if (!match2) {
      return errorResponse(`Route not found: ${method} ${apiPath}`, 404);
    }
    return await match2.route.handler(context, match2.params);
  } catch (error) {
...
// .wrangler/tmp/bundle-ijMbkY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ijMbkY/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9189860945503252.js.map
