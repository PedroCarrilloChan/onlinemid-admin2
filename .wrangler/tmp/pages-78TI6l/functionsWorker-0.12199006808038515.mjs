...
var onRequestGet = /* @__PURE__ */ __name(async (context2) => {
  try {
    ...
    const site = await siteQuery.bind(domain2).first();
    if (!site) {
    ...
    console.error("Error en API p\xFAblica:", error3);
    return new Response(JSON.stringify({ error: "Error interno del servidor" }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
...
var onRequest = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    if (request.method === "GET") {
      const result = await env2.DB.prepare("SELECT * FROM Clientes WHERE id = ?").bind(id).first();
    ...
    if (request.method === "PUT") {
      ...
      await env2.DB.prepare("UPDATE Clientes SET nombre = ?, email_contacto = ? WHERE id = ?").bind(body.nombre, body.email_contacto, id).run();
      return new Response(JSON.stringify({ id, ...body }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (request.method === "DELETE") {
      await env2.DB.prepare("DELETE FROM Clientes WHERE id = ?").bind(id).run();
      return new Response(null, { status: 204, headers: corsHeaders });
    }
    return new Response("M\xE9todo no permitido", { status: 405, headers: corsHeaders });
  } catch (error3) {
    console.error(`Error en API para ID ${id}:`, error3);
    return new Response(JSON.stringify({ error: "Error interno del servidor", details: error3.message }), {
      status: 500,
...
var onRequestGet2 = /* @__PURE__ */ __name(async (context2) => {
  try {
    const debugInfo = {
      ...
      nodeEnv: "undefined",
      request: {
        url: context2.request?.url || "unknown",
        method: context2.request?.method || "unknown"
      },
      context: {
        hasEnv: !!context2.env,
        hasParams: !!context2.params,
        hasData: !!context2.data
      },
    ...
    return new Response(JSON.stringify({
      ...
      message: error3 instanceof Error ? error3.message : "Error desconocido",
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    }, null, 2), {
      status: 500,
      headers: {
...
var onRequest2 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    if (request.method === "GET") {
      const result = await env2.DB.prepare("SELECT * FROM Clientes ORDER BY fecha_creacion DESC").all();
    ...
    console.error("Error en API:", error3);
    return new Response(JSON.stringify({ error: "Error interno del servidor", details: error3.message }), {
      status: 500,
      headers: { ...corsHeaders2, "Content-Type": "application/json" }
    });
...
var onRequest3 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    if (path.length === 1 && path[0] === "customers") {
      return await handleCustomersRoute(request, env2);
    }
    if (path.length === 2 && path[0] === "customers") {
      const customerId = path[1];
      return await handleCustomerByIdRoute(request, env2, customerId);
    }
...
async function handleCustomersRoute(request, env2) {
  if (request.method === "GET") {
    const result = await env2.DB.prepare("SELECT * FROM Clientes ORDER BY fecha_creacion DESC").all();
    return new Response(JSON.stringify(result.results || []), {
      headers: { ...corsHeaders3, "Content-Type": "application/json" }
...
async function handleCustomerByIdRoute(request, env2, customerId) {
  if (request.method === "GET") {
    const result = await env2.DB.prepare("SELECT * FROM Clientes WHERE id = ?").bind(customerId).first();
    if (!result) {
  ...
  if (request.method === "DELETE") {
    const result = await env2.DB.prepare("DELETE FROM Clientes WHERE id = ?").bind(customerId).run();
    if (!result.success) {
...
// ../.wrangler/tmp/pages-78TI6l/functionsRoutes-0.7690924534184402.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-GKLs5W/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-GKLs5W/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.12199006808038515.mjs.map
