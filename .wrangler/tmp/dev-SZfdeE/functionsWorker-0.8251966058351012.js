...
var onRequestPost = /* @__PURE__ */ __name2(async (context22) => {
  try {
    const { email, password } = await context22.request.json();
    if (!email || !password) {
...
var onRequestGet = /* @__PURE__ */ __name2(async (context22) => {
  try {
    ...
    const domain22 = url.searchParams.get("domain");
    if (!domain22) {
    ...
    const site = await siteQuery.bind(domain22).first();
    if (!site) {
    ...
    console.error("Error en API p\xFAblica:", error32);
    return new Response(JSON.stringify({ error: "Error interno del servidor" }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
...
var onRequest = /* @__PURE__ */ __name2(async (context22) => {
  ...
  try {
    if (request.method === "GET") {
      const result = await env22.DB.prepare("SELECT * FROM Clientes WHERE id = ?").bind(id).first();
    ...
    if (request.method === "PUT") {
      ...
      await env22.DB.prepare("UPDATE Clientes SET nombre = ?, email_contacto = ? WHERE id = ?").bind(body.nombre, body.email_contacto, id).run();
      return new Response(JSON.stringify({ id, ...body }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (request.method === "DELETE") {
      await env22.DB.prepare("DELETE FROM Clientes WHERE id = ?").bind(id).run();
      return new Response(null, { status: 204, headers: corsHeaders });
    }
    return new Response("M\xE9todo no permitido", { status: 405, headers: corsHeaders });
  } catch (error32) {
    console.error(`Error en API para ID ${id}:`, error32);
    return new Response(JSON.stringify({ error: "Error interno del servidor", details: error32.message }), {
      status: 500,
...
var onRequestGet2 = /* @__PURE__ */ __name2(async (context22) => {
  try {
    const debugInfo = {
      ...
      nodeEnv: "undefined",
      request: {
        url: context22.request?.url || "unknown",
        method: context22.request?.method || "unknown"
      },
      context: {
        hasEnv: !!context22.env,
        hasParams: !!context22.params,
        hasData: !!context22.data
      },
    ...
    return new Response(JSON.stringify({
      ...
      message: error32 instanceof Error ? error32.message : "Error desconocido",
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    }, null, 2), {
      status: 500,
      headers: {
...
var onRequest2 = /* @__PURE__ */ __name2(async (context22) => {
  ...
  try {
    if (request.method === "GET") {
      const result = await env22.DB.prepare("SELECT * FROM Clientes ORDER BY fecha_creacion DESC").all();
    ...
    return new Response("M\xE9todo no permitido", { status: 405, headers: corsHeaders2 });
  } catch (error32) {
    console.error("Error en API:", error32);
    return new Response(JSON.stringify({ error: "Error interno del servidor", details: error32.message }), {
      status: 500,
      headers: { ...corsHeaders2, "Content-Type": "application/json" }
    });
...
var onRequest3 = /* @__PURE__ */ __name2(async (context22) => {
  ...
  try {
    if (path.length === 1 && path[0] === "customers") {
      return await handleCustomersRoute(request, env22);
    }
    if (path.length === 2 && path[0] === "customers") {
      const customerId = path[1];
      return await handleCustomerByIdRoute(request, env22, customerId);
    }
...
async function handleCustomersRoute(request, env22) {
  if (request.method === "GET") {
    const result = await env22.DB.prepare("SELECT * FROM Clientes ORDER BY fecha_creacion DESC").all();
    return new Response(JSON.stringify(result.results || []), {
      headers: { ...corsHeaders3, "Content-Type": "application/json" }
  ...
  return new Response(JSON.stringify({ error: "M\xE9todo no permitido" }), {
    status: 405,
    headers: { ...corsHeaders3, "Content-Type": "application/json" }
  });
}
__name(handleCustomersRoute, "handleCustomersRoute");
__name2(handleCustomersRoute, "handleCustomersRoute");
async function handleCustomerByIdRoute(request, env22, customerId) {
  if (request.method === "GET") {
    const result = await env22.DB.prepare("SELECT * FROM Clientes WHERE id = ?").bind(customerId).first();
    if (!result) {
  ...
  if (request.method === "DELETE") {
    const result = await env22.DB.prepare("DELETE FROM Clientes WHERE id = ?").bind(customerId).run();
    if (!result.success) {
...
__name(handleCustomerByIdRoute, "handleCustomerByIdRoute");
__name2(handleCustomerByIdRoute, "handleCustomerByIdRoute");
...
// .wrangler/tmp/bundle-yUjN88/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yUjN88/middleware-loader.entry.ts
